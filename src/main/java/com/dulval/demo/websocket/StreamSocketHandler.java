...
import com.dulval.demo.util.Helpers;
import com.dulval.demo.util.SignalMessage;
...
public class StreamSocketHandler extends TextWebSocketHandler {

    private static final Logger LOG = LoggerFactory.getLogger(StreamSocketHandler.class);
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        connectedUsers.values().forEach(webSocketSession -> {
            try {
                webSocketSession.sendMessage(new TextMessage(Helpers.getString(newMenOnBoard)));
            } catch (Exception e) {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        SignalMessage signalMessage = Helpers.getObject(message.getPayload());
        ...
        if (destSocket != null && destSocket.isOpen()) {
            ...
            final String resendingMessage = Helpers.getString(signalMessage);
    ...
    private void removeUserAndSendLogout(final String sessionId) {
        ...
        connectedUsers.values().forEach(webSocket -> {
            try {
                webSocket.sendMessage(new TextMessage(Helpers.getString(menOut)));
            } catch (Exception e) {
                LOG.warn("Error while message sending.", e);
            }
        });
